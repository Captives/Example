function SingleRoom(){
    ...
    this.server.on(RemoteEvent.ER, function(data){
        that.dispose();
        that.emit("reject_online",data);
        console.log("拒绝登入", JSON.stringify(data));
    });
...
SingleRoom.prototype.sendCans = function(text){
    if(this.server){
        this.server.share(ShareEvent.FAQ_COUNT, text, null,UserType.TEACHER);
    }
};
