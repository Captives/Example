...
var Remote = require('./../resource/SingleRemoteResource');
...
function WebServer() {
    ...
    this.on('join', function (ws, data) {
        this.register(ws, data.room, data.userdata, data.clientIP, data.type);
    });

    this.on("share", function (ws, data) {
        if (ws.activated) {
            this.broadcast(ws.room, ws, data, Date.now());
        } else {
            this.emit("invalid_share_message", ws, data);
        }
    });

    this.on("offline", function (ws, data) {
        //断开其他的客户端
        this.offline(ws, data);
        //当前的客户端上线
        this.online(ws, "强制登入");
    });

    //查找目标socket和事件触发者交换candidate信息
    this.on("icecandidate", function (ws, data) {
        var socket = this.getSocket(data.wid);
        if(socket){
            this.remote.sendIceCandidate(socket, ws.id, data.label, data.candidate);
        }else{
            console.log("on ice candidate", data.wid, "已退出");
        }

        this.emit("iceCandidate", ws.userdata, data.wid, data.label, data.candidate);
    });

    //查找目标socket发送事件触发者的sdp
    this.on("offer", function (ws, data) {
        var socket = this.getSocket(data.wid);
        if(socket){
            this.remote.sendOffer(socket, ws.id, data.sdp);
        }else{
            console.log("on offer", data.wid, "已退出");
        }

        this.emit("sdp_offer", ws.userdata, data.wid, data.sdp);
    });

    //查找目标socket，发送事件触发者的sdp
    this.on("answer", function (ws, data) {
        var socket = this.getSocket(data.wid);
        if(socket){
            this.remote.sendAnswer(socket, ws.id, data.sdp);
        }else{
            console.log("on answer", data.wid, "已退出");
        }
        this.emit("sdp_answer", ws.userdata, data.wid, data.sdp);
    });
...
WebServer.prototype.online = function (ws, status) {
    ...
    phpResource.logAction(ws.room, ws.userdata, ws.userdata, ShareEvent.USER_ENTER,
        status + "(" + ws.clientIP + "[" + this.devInfo(ws.loginType)+ "])", Date.now());
...
WebServer.prototype.broadcast = function (room, ws, data, time) {
    ...
    function unRepeat(list){
        ...
        for(var i=0; i < list.length; i++){
            ...
            if(j == -1){
                ...
                phpResource.statusAction(room, ws.userdata, list[i].userdata, data, time);
            }
        }
        return temp;
    };
    ...
    if (list.length == 0 && direct == false) {
        ...
        phpResource.statusAction(room, ws.userdata, null, data, time);
    } else {
        //console.log("定向发送消息");
        this.broadcaseInGroup(list, ws, data, time);
    }

    if (data.action !== ShareEvent.CURSOR_POSITION){//鼠标移动的事件，暂不输出
        this.debug("share", ws, data);
        this.emit('socket_command', ws.room, room, ws.userdata, data);
    }
};
...
WebServer.prototype.removeSocketByRoom = function (ws, text) {
    ...
    phpResource.logAction(ws.room, ws.userdata, ws.userdata, ShareEvent.USER_QUIT,
        text + "(" + ws.clientIP + "[" + this.devInfo(ws.loginType)+ "])", Date.now());
...
WebServer.prototype.init = function (ws) {
    ...
    ws.on('close', function () {
        that.socketClose(ws);
        that.emit("socket_close", ws.id);
    });

    ws.on('error', function (error) {
        that.emit('socket_error', error, uid);
    });

    that.addSocket(ws);
    that.emit("new_connect", ws.id);
};
...
module.exports.start = function (options, path, host, port) {
    ...
    console.log("WebsocketServer clients",ws.clients);
    return ws.webServer;
};
